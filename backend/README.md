...
- **به‌روزرسانی صادقانه (پس از یک Deploy واقعی روی VPS کاربر):** فرض اولیهٔ بالا («MariaDB و MySQL کاملاً یکسان رفتار می‌کنند») غلط از آب درآمد. وقتی یک کاربر واقعاً این پروژه را با `docker compose` روی یک VPS با ایمیج رسمی `mysql:8.0` بالا آورد (نه MariaDB)، Migration با ۲ خطای واقعی مخصوص MySQL شکست خورد که هرگز روی MariaDB 10.11 دیده نشده بودند:
  - `ER_BLOB_CANT_HAVE_DEFAULT`: MySQL 8.0 برخلاف MariaDB، به ستون‌های `TEXT` اجازهٔ `DEFAULT` نمی‌دهد (چند ستون Enum مثل `pay_type`, `status` این‌طور بودند؛ به `VARCHAR(20)` تغییر کردند).
  - `ER_BLOB_KEY_WITHOUT_LENGTH`: MySQL 8.0 برخلاف MariaDB، اجازه نمی‌دهد روی یک ستون `TEXT` بدون طول مشخص Index ساخته شود (`audit_logs.action`؛ به `VARCHAR(64)` تغییر کرد).
  این‌بار برای اطمینان، همان ۹۰ تست را روی یک نمونهٔ **MySQL 8.0.46 واقعی** (نه MariaDB) هم اجرا کردیم — همه پاس شدند. جزئیات کامل رفع این دو باگ در تاریخچهٔ Commit همین مخزن است.
- چیزی که هنوز **واقعاً اجرا نشد**: یک هاست اشتراکی واقعی (DirectAdmin/cPanel) با MySQL/MariaDB مدیریت‌شده توسط پنل هاست (بجای یک نمونهٔ خودمدیریتی نصب‌شده با Docker/apt) — تفاوت‌های احتمالی تنظیمات پیش‌فرض چنین پنل‌هایی هنوز به‌صورت مستقیم تست نشده.
