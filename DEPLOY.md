...
| `web/index.html` | پیش‌نمایش اپ کاربران — **مستقل است، به Backend وصل نیست** (در مرورگر خودش با `localStorage` کار می‌کند) | هرجا، فقط برای دیدن ظاهر UI |
...
فقط آپلود کنید و باز کنید — به هیچ تنظیمی نیاز ندارد (چون به Backend وصل نیست، طبق جدول بالا).
