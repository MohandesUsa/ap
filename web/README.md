...
- ثبت‌نام/ورود، کامیون‌ها، دعوت راننده، سرویس‌ها، هزینه‌ها، و تسویه‌حساب (تنظیم کمیسیون، حقوق راننده، پرداخت دستی) دیگر Mock نیستند: همهٔ این‌ها به یک دیتابیس واقعی وصل‌اند و با رفرش صفحه از بین نمی‌روند. وقتی این صفحه داخل یک Claude Artifact باز شود از دیتابیس Real-time و مشترک همان پلتفرم استفاده می‌کند (باز کردن دو تب — یکی صاحب کامیون، یکی راننده — همدیگر را زنده می‌بینند)؛ وقتی روی سرور خودتان (طبق همین فایل) اجرا شود، به‌صورت خودکار روی `localStorage` مرورگر Fallback می‌کند — یعنی هر کاربر داده‌های خودش را در همان مرورگر نگه می‌دارد، نه مشترک بین کاربران.
- این دیتابیس مستقل از `backend/` واقعی پروژه است — همان محاسبات (سهم راننده، مانده بدهی) اینجا مستقیماً در جاوااسکریپت همین صفحه انجام می‌شود، نه با صدا زدن API واقعی؛ برای اتصال واقعی به `backend/`، اپلیکیشن Android (طبق `README.md` ریشهٔ پروژه، بخش Phase 3) همان کاری است که باید انجام شود.
