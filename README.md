...
**`web/index.html` (پیش‌نمایش اپ Owner/Driver) هم اکنون واقعاً به همین Backend وصل است** — دیگر `localStorage`/Firestore Mock نیست؛ ثبت‌نام و ورود واقعی (با `deviceId` و جریان کامل تأیید گوشی جدید)، کامیون‌ها، دعوت راننده (بدون نیاز راننده به وارد کردن کد — دعوت‌نامه بعد از ورود خودش در Dashboard راننده نشان داده می‌شود، دقیقاً مثل اپ اندروید)، ثبت سرویس/هزینه، تسویه‌حساب و ثبت پرداخت، و ویرایش پروفایل، همگی روی endpointهای واقعی `backend/` کار می‌کنند. با Playwright روی یک نمونهٔ واقعی از Backend + MySQL 8.0 تست شد (۲۲ سناریوی واقعی، شامل کل جریان Owner→Truck→Invite→Driver→Accept→Trip→Expense→Settlement→Payment، و جریان کامل تأیید ورود از گوشی دوم). جزئیات در `web/README.md`.
...
- نسخهٔ پیش‌نمایش وب (`web/index.html`) هم برای ثبت‌نام/ورود، کامیون‌ها، دعوت راننده، سرویس‌ها، هزینه‌ها و تسویه‌حساب به همین `backend/` واقعی (نه یک دیتابیس مستقل) وصل شده (جزئیات در `web/README.md`).
